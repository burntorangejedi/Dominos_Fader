...
addonTable.modules.extra = ExtraBarModule

function ExtraBarModule:RegisterBar(bar, registrationData)
    local moduleName = "Dominos_ExtraBar"
    local eventName = "DOMINOS_EXTRA_BAR_UPDATE"
    
    -- For extra ability bar, we need to get the ExtraActionBarFrame button
    local buttons = {}
    ...
    if ZoneAbilityFrame and ZoneAbilityFrame.SpellButton then
        table.insert(buttons, ZoneAbilityFrame.SpellButton)
    end
    
    -- Also check bar buttons if any
    if bar.buttons then
...
function ExtraBarModule:UnregisterBar(bar)
    local moduleName = "Dominos_ExtraBar"
